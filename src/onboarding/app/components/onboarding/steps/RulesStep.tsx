...
export function RulesStep({ onBack, onNext, nextLabel, nextDisabled }: StepProps) {
  ...
  return (
    <OnboardingLayout onBack={onBack} onNext={onNext} nextLabel={nextLabel} nextDisabled={nextDisabled}>
      <OnboardingCard>
            ...
            {explainerOpen && (
              <div id={verifiedExplainerId} className={`mt-2 ${obQuote}`} role="region">
                ...
                <p className={`mt-1.5 ${obBody}`}>
                  <Link to="/verify" className="font-medium text-teal underline-offset-2 hover:underline">
