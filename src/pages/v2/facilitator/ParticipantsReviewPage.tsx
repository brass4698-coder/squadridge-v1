...
const SEED_PARTICIPANTS: Participant[] = [
  { id: 'p1', codename: 'Participant A', invitedAt: '2026-06-17 09:14', verificationStatus: 'verified', documentSubmitted: true },
  { id: 'p2', codename: 'Participant B', invitedAt: '2026-06-17 09:21', verificationStatus: 'pending', documentSubmitted: true },
  { id: 'p3', codename: 'Participant C', invitedAt: '2026-06-17 10:05', verificationStatus: 'pending', documentSubmitted: false },
];
...
export function ParticipantsReviewPage() {
  ...
  function setStatus(id: string, status: VerificationStatus) {
    setParticipants((ps) => ps.map((p) => p.id === id ? { ...p, verificationStatus: status } : p));
  }
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <div className="mb-8 flex items-start justify-between gap-4">
          <div>
            ...
            <h1 className="text-xl font-semibold tracking-tight" style={{ color: 'var(--color-text-primary)' }}>Participant review</h1>
            <p className="mt-1 text-sm" style={{ color: 'var(--color-text-secondary)' }}>Review submitted documents and approve or deny each participant before starting the session.</p>
          </div>
          <button
            disabled={!allVerified}
            onClick={() => navigate(`/f/sessions/${sessionId}/control`)}
          ...
          {participants.map((p) => (
            ...
            >
              <div className="mb-4 flex items-center justify-between gap-4">
                <div>
                  <p className="text-sm font-semibold" style={{ color: 'var(--color-text-primary)' }}>{p.codename}</p>
                  <p className="text-xs" style={{ color: 'var(--color-text-secondary)' }}>Invited {p.invitedAt}</p>
                </div>
              ...
              <div
                ...
                style={{
                  backgroundColor: p.documentSubmitted ? 'var(--color-accent-light)' : 'var(--color-border)',
                  color: p.documentSubmitted ? 'var(--color-accent)' : 'var(--color-text-secondary)',
                }}
              >
                {p.documentSubmitted ? '✓ Identity document submitted' : '○ No document submitted yet'}
              </div>
