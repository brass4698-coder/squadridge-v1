...
function RequiredMark() {
  return (
    <span className="ml-1.5 align-middle font-sans text-[0.65rem] font-bold uppercase tracking-[0.14em] text-sem-info">
      Required
    </span>
  );
}
...
function OptionalMark() {
  return (
    <span className="ml-1.5 align-middle font-sans text-[0.6rem] font-normal uppercase tracking-[0.08em] text-ink-subtle">
      Optional
    </span>
  );
}
...
export function ProfileSettingsPage() {
  ...
  if (!isSupabaseConfigured()) {
    return (
      <div className="mx-auto max-w-copy px-gutter py-12">
        ...
        <Link to="/" className="mt-4 inline-block text-brand underline-offset-4 hover:underline">
          Home
        </Link>
      </div>
    );
  ...
  return (
    <div className="relative mx-auto w-full max-w-[560px] px-gutter py-12">
      <div
        className="pointer-events-none absolute inset-x-0 top-0 z-0 h-[min(36vh,22rem)] bg-[radial-gradient(ellipse_90%_70%_at_50%_-10%,rgba(42,163,154,0.07)_0%,transparent_58%)]"
        aria-hidden
      />
      <div className="relative z-[1]">
        ...
        <div className="mt-2 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <h1 className="font-heading text-fluid-h2 font-extrabold leading-[1.12] tracking-[-0.03em] text-ink">
            Profile &amp; keys
          </h1>
          {isDirty ? (
            <p
        ...
        <p className="mt-3 max-w-[52ch] font-sans text-[0.9rem] leading-relaxed text-ink-subtle">
        ...
        >
          ...
          >
            <div>
              ...
              <p className="mt-1.5 max-w-[52ch] font-sans text-[0.8rem] leading-relaxed text-ink-subtle">
            ...
            <div className="space-y-2">
              ...
              <p
                id="pf-callsign-hint"
                className="font-sans text-[0.78rem] leading-relaxed text-ink-subtle"
              >
              ...
              <p
                ...
                className={
                  role && role !== 'other' && ROLE_HINT[role]
                    ? 'font-sans text-[0.78rem] leading-relaxed text-ink-subtle'
                    : role === 'other'
                      ? 'font-sans text-[0.78rem] leading-relaxed text-ink-subtle'
                      : 'sr-only'
                }
            ...
            <div className="space-y-2">
              ...
              <p className="font-sans text-[0.78rem] leading-relaxed text-ink-subtle">
              ...
              {tagsList.length > 0 ? (
                <ul className="m-0 flex list-none flex-wrap gap-1.5 p-0" aria-label="Parsed tags">
                  {tagsList.slice(0, MAX_TAGS).map((t) => (
                    <li
                      key={t}
                      className="inline-flex max-w-full truncate rounded-md border border-line-strong bg-surface-elevated px-2 py-0.5 font-mono text-[0.68rem] text-ink-faint"
                    >
                      {t}
                    </li>
                  ))}
              ...
              <input
                ...
                className={`sr-input ${
                  tagsTooMany ? 'border-amber/50 focus-visible:border-amber/60' : 'border-line'
                }`}
          ...
          <section className="glass-card space-y-6 p-6 sm:p-8" aria-labelledby="pf-routing-heading">
            <div>
              <h2
              ...
              <p className="mt-2 max-w-[52ch] font-sans text-[0.8rem] leading-relaxed text-ink-subtle">
                <strong className="font-semibold text-ink">Used for matching only</strong>
                <span className="text-ink-subtle"> — </span>
                <span className="text-ink-faint">not shown to squads.</span>
              </p>
              <details className="group mt-3 rounded-lg border border-line bg-surface-sunken px-3 py-2">
                <summary className="cursor-pointer list-none font-sans text-[0.78rem] font-medium text-brand [&::-webkit-details-marker]:hidden">
                  <span className="underline decoration-teal/30 underline-offset-2 group-open:decoration-teal/60">
                    Why we ask for this
                  </span>
                </summary>
                <p className="mt-2 border-t border-line/80 pt-2 font-sans text-[0.78rem] leading-relaxed text-ink-subtle">
        ...
        {supabase ? (
          <section className="glass-card mt-10 p-6">
            ...
            <p className="mt-2 font-sans text-[0.78rem] leading-relaxed text-ink-subtle">
            ...
            <div className="mt-5 grid gap-6 sm:grid-cols-2">
              <div className="space-y-3">
                <p className="font-sans text-[0.72rem] font-semibold uppercase tracking-[0.1em] text-ink-subtle">
                ...
                <button
                  ...
                  className="sr-input inline-flex min-h-[40px] w-full items-center justify-center transition hover:bg-surface-elevated disabled:opacity-60"
                >
                  {demoClaimBusy ? 'Generating…' : 'Generate transfer code'}
                </button>
                {demoClaimCode ? (
                  <p className="break-all rounded-[6px] border border-line bg-surface-sunken px-3 py-2 font-mono text-[0.8rem] text-brand">
                    {demoClaimCode}
                  </p>
                ) : null}
                {demoClaimErr ? (
                  <p className="font-sans text-[0.8rem] text-sem-danger">{demoClaimErr}</p>
                ) : null}
              </div>
              <div className="space-y-3">
                <p className="font-sans text-[0.72rem] font-semibold uppercase tracking-[0.1em] text-ink-subtle">
                ...
                <input
                  ...
                  className="sr-input font-mono text-[0.85rem]"
                />
                <button
                  type="button"
                  disabled={finalizeBusy || !finalizeCode.trim()}
                  onClick={() => void handleRequestConsent()}
                  className="focus-ring btn-primary inline-flex min-h-[40px] w-full items-center justify-center px-4 font-heading text-[0.88rem] font-semibold disabled:cursor-not-allowed disabled:opacity-50"
                >
                ...
                {finalizeErr ? (
                  <p className="font-sans text-[0.8rem] text-sem-danger">{finalizeErr}</p>
                ) : null}
              </div>
            </div>
          </section>
        ...
        <NextStepHint className="mt-8 border-line bg-surface-elevated/60">
        ...
        <div
          className="pointer-events-none fixed inset-x-0 bottom-0 z-20 flex justify-center border-t border-line bg-surface-sunken/95 px-gutter py-3 backdrop-blur-md sm:px-6"
        ...
        >
          <div className="pointer-events-auto flex w-full max-w-[560px] flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
            ...
            <button
              ...
              className="focus-ring btn-primary inline-flex min-h-[44px] w-full shrink-0 items-center justify-center px-8 py-3 font-heading text-[0.95rem] font-semibold disabled:cursor-not-allowed disabled:opacity-50 sm:ml-auto sm:w-auto"
            >
        ...
        <p className="mt-10 font-sans text-[0.85rem] text-ink-subtle">
          <Link
            to="/"
            onClick={confirmLeaveInApp}
            className="focus-ring text-ink-faint underline-offset-4 hover:text-ink-secondary hover:underline"
          >
            Back to home
          </Link>
        </p>
