...
export function SessionStrategyRoomChrome({
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 rounded-lg bg-surface-elevated p-4 shadow-sr-card sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <p className="font-heading text-[0.65rem] font-semibold uppercase tracking-[0.12em] text-amber/90">
            Strategy room
          </p>
          <h2 className="mt-0.5 break-words font-heading text-lg font-semibold text-ink">
            {topic}
          </h2>
        </div>
        <div className="shrink-0 text-right">
          <p className="font-sans text-[0.7rem] uppercase tracking-wide text-ink-faint">
            Time in room
          </p>
          <p className="font-mono text-[0.9rem] tabular-nums text-brand/90" aria-live="polite">
            {elapsed}
          </p>
        </div>
      </div>
      ...
      <div className="rounded-lg bg-surface-secondary px-4 py-3 shadow-sr-sm">
        <p className="font-sans text-[0.7rem] font-semibold uppercase tracking-wide text-ink-faint">
          Turn
        </p>
        <p className="mt-1 font-sans text-[0.9rem] leading-relaxed text-ink">{turnCta}</p>
      </div>

      <div className="flex flex-wrap items-center gap-2">
        <span className="font-sans text-[0.7rem] uppercase tracking-wide text-ink-faint">
          Reactions
        </span>
        {REACTIONS.map((r) => (
          <button
            key={r}
            type="button"
            className="rounded-md bg-surface-elevated px-2.5 py-1 font-sans text-[0.75rem] text-ink-secondary shadow-sr-xs hover:border-brand/35 hover:text-ink"
            onClick={() => {
      ...
      {interventionBanner ? (
        <div
          className="rounded-lg border border-amber/35 bg-sem-warning-soft px-4 py-3 font-sans text-[0.85rem] text-sem-warning"
          role="status"
        >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-line pt-3">
        <div className="flex flex-wrap gap-2">
          <Link
            to="/session"
            className="inline-flex min-h-[40px] items-center rounded-md border border-line-strong px-3 font-sans text-[0.8rem] text-ink-secondary hover:border-brand/35"
          >
            Exit to hub
          </Link>
          <button
            ...
            className="inline-flex min-h-[40px] items-center rounded-md border border-amber/25 px-3 font-sans text-[0.8rem] text-sem-warning hover:border-amber/45"
          >
            Report room
          </button>
          <button
        ...
        <span className="max-w-[14rem] text-right font-sans text-[0.7rem] leading-snug text-ink-faint">
