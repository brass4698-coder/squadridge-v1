...
export function ProcessStagePanel() {
  return (
    <section
      id="system-model"
      className="scroll-mt-20 border-b border-[color:var(--color-border-subtle)] bg-surface-sunken/35 py-16 md:py-24"
    ...
    >
      <div className={publicShellInnerClass}>
        ...
        <ol className="m-0 mt-10 grid list-none gap-3 p-0 sm:grid-cols-3 sm:gap-4">
          {STATES.map((state) => (
            <li key={state.num}>
              <a
                href={state.href}
                className="flex h-full flex-col rounded-lg border border-[color:var(--color-border-subtle)] bg-surface-elevated px-4 py-4 no-underline transition-colors hover:border-brand/40"
              >
                ...
                <span className="mt-3 text-sm font-semibold tracking-tight text-ink">
                  {state.label}
                </span>
              </a>
            </li>
