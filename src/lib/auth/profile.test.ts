...
import {
  isProfileComplete,
  PROFILE_ROLE_OTHER_MAX_LEN,
  PROFILE_ROLE_OTHER_MIN_LEN,
} from './profile';
...
describe('isProfileComplete', () => {
  ...
  it('when role is other, requires role_other_detail in [min,max] trimmed length', () => {
    ...
    expect(isProfileComplete(base({ role_archetype: 'other', role_other_detail: short }))).toBe(
      false,
    );
    expect(isProfileComplete(base({ role_archetype: 'other', role_other_detail: ok }))).toBe(true);
    expect(isProfileComplete(base({ role_archetype: 'other', role_other_detail: long }))).toBe(
      false,
    );
  });
});
