...
- [ ] **Platform audit (Team/Enterprise only):** if the org plan includes Platform Audit Logs, confirm who can open org audit history and whether an [Audit Log Drain](https://supabase.com/docs/guides/security/platform-audit-logs) to an owned destination is required for the pilot/partner. This is org dashboard/API actions — not `moderation_audit_log` or Auth audit logs. Skip on Free/Pro until plan upgrade.
- [ ] **GraphQL:** app and CI use PostgREST + generated TS types only. Do not enable pg_graphql introspection for production unless a deliberate GraphQL client is added (introspection is off by default as of pg_graphql v1.6.2).
