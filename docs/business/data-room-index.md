# Data Room Index

This index is a suggested structure for a lightweight diligence room. It helps partners and funders find the current, engineering-aligned materials first.

## Start Here

- Current state: [`../../CURRENT_STATUS.md`](../../CURRENT_STATUS.md)
- Diligence summary: [`../../DILIGENCE_OVERVIEW.md`](../../DILIGENCE_OVERVIEW.md)
- Repository overview: [`../../README.md`](../../README.md)

## Product

- Product overview: [`../product/product-overview.md`](../product/product-overview.md)
- Feature specifications: [`../product/feature-specifications.md`](../product/feature-specifications.md)
- User journeys: [`../product/user-journeys.md`](../product/user-journeys.md)
- Metrics specification: [`../product/metrics-spec.md`](../product/metrics-spec.md)

## Security And Architecture

- Threat model: [`../security/threat-model.md`](../security/threat-model.md)
- Security/privacy overview: [`../technical/security-privacy.md`](../technical/security-privacy.md)
- Architecture overview: [`../technical/architecture-overview.md`](../technical/architecture-overview.md)
- ZK implementation: [`../technical/zk-implementation.md`](../technical/zk-implementation.md)

## Operations

- Production checklist: [`../operations/production-checklist.md`](../operations/production-checklist.md)
- Incident response: [`../operations/incidents.md`](../operations/incidents.md)
- Pilot runbook: [`../operations/pilot-runbook.md`](../operations/pilot-runbook.md)
- Data retention guidance: [`../operations/data-retention-operators.md`](../operations/data-retention-operators.md)

## Business And Partnerships

- Pilot partner one-pager: [`./pilot-partner-one-pager.md`](./pilot-partner-one-pager.md)
- Go-to-market: [`./go-to-market.md`](./go-to-market.md)
- Impact metrics: [`./impact-metrics.md`](./impact-metrics.md)
- Partnership strategy: [`./partnership-strategy.md`](./partnership-strategy.md)
- Revenue model: [`./revenue-model.md`](./revenue-model.md)

## Recommended Additions Outside The Repo

These may live in a private diligence folder rather than git:

- legal/privacy drafts
- budget and use-of-funds
- cap table or financing history
- partner pipeline or LOIs
- pilot reports and testimonials
- external security review artifacts
