...
- **Peer-visible pseudonyms (in-squad only):** Squad members may query each other’s `callsign`, `role_archetype`, `tags`, and `region_hint` via the `get_squad_peer_profiles(squad_id)` SECURITY DEFINER RPC (`supabase/migrations/20260419120000_squad_peer_profiles_rpc.sql`). The function verifies squad membership before returning rows — **no cross-squad directory exists**; a user cannot enumerate profiles outside squads they belong to. The `profiles_select_own` RLS policy remains unchanged (own-row only). Operators with service-role or dashboard access can still read all profile data directly.
...
| 2026-04-19 | §5: Added peer-visible pseudonyms — `get_squad_peer_profiles` RPC (in-squad only, no global directory). |
