...
- **Messages use AES-256-GCM at the application layer (payload v3), not end-to-end encryption against the platform.** `src/lib/messageCrypto.ts` encrypts each message body with a random 12-byte IV; ciphertext lives in `messages.payload_ciphertext` as JSON (`v`, `alg`, `iv`, `ct`). The column name reflects **ciphertext**, not a claim of operator-proof E2E. The **symmetric squad key** is now organised into per-squad **epochs**: `public.squad_key_epochs` holds one row per epoch; `squads.current_epoch_id` points at the live row; `messages.key_epoch_id` stamps which epoch encrypted that row. `squads.message_encryption_key` is preserved as a denormalised fast-path mirror of the current epoch's key. Any member who can `SELECT` `squad_key_epochs` (RLS allows squad members and moderators) can decrypt; **moderators** and anyone with **service role** or raw DB access can read keys + ciphertext. **Key rotation is now productized:** `public.rotate_squad_key(p_squad_id, p_reason)` (`20260502120100_rotate_squad_key_rpc.sql`) is a moderator-callable, audited RPC that mints a new 32-byte AES key, retires the previous epoch, and writes a `moderation_audit_log` row (`action=squad_key_rotated`). **Interim forward secrecy** ships in `20260503120000_purge_retired_squad_keys.sql` + `20260503120100_archive_purges_epoch_keys.sql`: a daily pg_cron purges retired epochs of live squads after 30 days; archiving a squad immediately purges all per-epoch keys for it, leaving `squads.archived_encryption_key_snapshot` as the single moderator-recovery surface. This is **not** Signal-grade FS — it does not protect against an operator who copies keys before purge — but it bounds the future-DB-snapshot attack window. **True E2E** (unreadable by Supabase/operators) would require per-user key distribution (Signal-style / MLS) and remains deferred per [ADR 004](../adr/004-defer-operator-blind-e2e.md). **New squads get a server-generated key** if the client omits it: migration `20260417150000_squads_message_encryption_key_server_default.sql` installs a `BEFORE INSERT` trigger using `pgcrypto`; `20260502120300_squad_key_epoch_seed_trigger.sql` adds an `AFTER INSERT` trigger that seeds epoch 1 and points `current_epoch_id` at it. `ensureSquadMessageKey` in `src/lib/crypto/squadMessageKey.ts` remains a client-side fallback for empty keys.
...
- **Squad encryption snapshot archive.** Migration `20260428123000_archive_squad_encryption_snapshot.sql` introduced `squads.archived_encryption_key_snapshot`; the v2 archive flow (`20260502120200_archive_squad_uses_epochs.sql` and `20260503120100_archive_purges_epoch_keys.sql`) snapshots the **current epoch's** key into that column and purges all per-epoch keys for the squad in the same transaction. Moderator decrypt-for-review reads from the snapshot column for archived squads, the current epoch's key for live squads, and the message's stamped epoch for old in-flight rotations — see `src/lib/moderation/modDecrypt.ts`.
...
- **Rate limiting:** Hot RPC and Edge actions are gated by `assertEdgeRateLimit(supabase, action)` in [`src/lib/rateLimitEdge.ts`](../../src/lib/rateLimitEdge.ts), which calls the `rate-limit` Edge Function backed by Upstash. There is **no separate client-side `anonymizeRequest` / `rateLimiter` middleware** in this app — client requests don't carry IPs the SPA can intercept; IP handling is delegated to the Supabase platform and the Upstash limiter. Until a Supabase-platform IP-anonymisation control is documented and exercised, the threat model treats vendor-side IP exposure as part of the **§3 tier 5 (platform vendor)** residual risk, not as an in-app control.
...
| 2026-05-01 | §5: Added explicit rate-limiting + IP-handling note. The codebase has **no** `src/middleware/anonymizeRequest.ts` or `src/middleware/rateLimiter.ts` — rate limits flow through `assertEdgeRateLimit` → `rate-limit` Edge Function (Upstash); vendor-side IP exposure stays a §3 tier-5 residual risk. Pairs with `docs/tech-debt.md #14` (review correction). |
| 2026-05-02 | §5: **Productized squad key rotation** (Track A) — new `squad_key_epochs` table (`20260502120000`), `rotate_squad_key` audited RPC (`20260502120100`), epoch seeding trigger (`20260502120300`), and updated archive flow (`20260502120200`). Replaces the "no key rotation in the product today" claim. ADR 004 (defer operator-blind E2E) still stands; this is a within-bounds rotation primitive, not E2E. |
| 2026-05-03 | §5: **Interim forward secrecy** (Track B) — daily pg_cron purges retired-epoch keys for live squads after 30 days (`20260503120000`); archiving a squad immediately drops per-epoch keys and keeps the recovery copy in `archived_encryption_key_snapshot` only (`20260503120100`). Bounds the future-DB-snapshot attack window; does not protect against operators who copy keys before purge. |
