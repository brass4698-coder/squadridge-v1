...
- **v2 session rooms store dialogue as plaintext.** `public.session_messages.body` is a plain `text` column (`20260618_squadridge_v2_schema.sql`); the v2 facilitated-room path has **no application-layer encryption at all**, unlike the legacy squad `messages` path described above. RLS scopes Data API reads to the session's facilitator, and participants reach the room only through token-validated `security definer` RPCs — but **service-role and direct DB access read room content in the clear**. Public copy must keep saying operator-readable; see `SecurityPage` “Who can read the room”.
