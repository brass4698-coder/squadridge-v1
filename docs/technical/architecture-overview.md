...
*   **Matchmaking and queues**: Implemented with Postgres-backed RPCs and tables (`matchmaking_*`), not Redis. Optional Redis or dedicated workers may be considered later for extreme scale; there is no local Redis stub in the repo today. (Earlier revisions shipped a `docker-compose.yml` Redis service that was never wired; it was removed as dead infrastructure.)
